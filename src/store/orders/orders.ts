// ** Redux Imports
import { IOrderState, OrderType } from '@/interfaces/orders'
    ...
    reducers: {
        ...
        searchOrder: (state, action) => {
            if (action.payload === '') {
                ...
                const filteredData = orders.filter(el => `${el.id}`.includes(`${action.payload}`))
...
export const { updateOrder, searchOrder } = orderSlice.actions
